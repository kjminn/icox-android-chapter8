    ...
    val eventListener: SensorEventListener = object : SensorEventListener {
        ...
        override fun onSensorChanged(event: SensorEvent?) {
            event?.let {
                ...
                // 각 좌표값을 제곱하여 음수값을 없애고, 갑의 차이를 극대화
                val power =
                        Math.pow(event.values[0].toDouble(), 2.0) + Math.pow(event.values[1].toDouble(), 2.0) + Math.pow(
    ...
    fun initGame() {
        ...
        animation.setAnimationListener(object : Animation.AnimationListener {
            override fun onAnimationRepeat(animation: Animation?) {
                ...
                Toast.makeText(applicationContext, "애니메이션 반복", Toast.LENGTH_SHORT).show()
            }

            override fun onAnimationEnd(animation: Animation?) {
                // 애니메이션이 종료될때의 코드를 작성
                Toast.makeText(applicationContext, "애니메이션 종료", Toast.LENGTH_SHORT).show()
            }

            override fun onAnimationStart(animation: Animation?) {
                // 애니메이션이 시작될때의 코드를 작성
                Toast.makeText(applicationContext, "애니메이션 시작", Toast.LENGTH_SHORT).show()
            }
        })
